...
class FirstOrderRandomWalker:
    ...
    def simulate_walks(self):
        ...
        """
        self.walks = []
        for iteration in range(self.num_walks):#10次
            print("\nRandom walk round: "+str(iteration+1)+"/"+str(self.num_walks)+".\n")
            for node in tqdm(self.G.nodes()):
                walk_from_node = self.do_walk(node)
                self.walks.append(walk_from_node)#
            print('random walking generate type',type(self.walks),'random walk node type',len(self.walks))
