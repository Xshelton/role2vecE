...
def parameter_parser():
    ...
    parser.add_argument('--graph-input',
                        nargs = '?',
                        default = "./input/BIOGRID-ALL-4.1.190(no repeate)10.csv",
	                help = 'Input graph path -- edge list csv.')

    parser.add_argument('--output',
                        nargs = '?',
                        default = './output/BIOGRID-ALL-4.1.190(no repeate)10embedding.csv',
...
def main(args):
    ...
    model = Role2Vec(args)
    model.do_walks()
    print('finished do walks')
    model.create_structural_features()
    print('second step feature extraction finished')
    model.learn_embedding()
    print('finished embedding generation')
    model.save_embedding()
    print('finished embedding saving')
