...
class MotifCounterMachine(object):
    """
    Motif and Orbit Counting Tool.轨道与motif计算工具
    """
    ...
    def enumerate_graphs(self):
        """
        Enumerating connected benchmark graphlets.枚举连接的基准图形
        """
    ...
    def enumerate_categories(self):
        """
        Enumerating orbits in graphlets.在图解中枚举轨道。
        """
        main_index = 0
            ...
            for nodes in tqdm(node_lists):
                ...
                for index, graph in enumerate(graphs):
                    if nx.is_isomorphic(sub_gr, graph):
                        ...
                        break
        print('特征构造',type(self.features),len(self.features))
        print(type(self.features),len(self.features),self.features[0])
        print(self.features)


    def create_tabular_motifs(self):
        """
        Creating tabular motifs for factorization.创建因式分解的表格模式
        """
        ...
        print(self.unique_motif_count)
        for index in range(self.unique_motif_count):
            features = self.motifs["role_"+str(index)].values.tolist()
            if sum(features)>0:#如果特征大于0才做
                features = [math.log(feature+1) for feature in features]
                print(len(features),type(features),features)
                features = pd.qcut(features,self.args.quantiles,duplicates="drop", labels=False)#qcut是根据这些值的频率来选择箱子的均匀间隔
                print(len(features),type(features),features)
                for node in self.graph.nodes():
                    self.binned_features[node].append(str(int(index*self.args.quantiles + features[node])))
        print('特征构造_tabular',type(self.binned_features),len(self.binned_features))
        print(type(self.binned_features),len(self.binned_features),self.binned_features[0])
        print(self.motifs)
    ...
    def create_string_labels(self):#上一步引用的创造string label
        """
        Executing the whole label creation mechanism.
        """
        print('motif查询-----构建子图部分------')
        self.create_edge_subsets()
        print('motif计数-----枚举子图部分------')
        self.enumerate_graphs()
        print('motif计数-----枚举类型部分------')
        self.enumerate_categories()
        print('motif计数-----初始化特征部分------')
        self.setup_features()
        print('motif计数-----对数装箱特征部分------')
